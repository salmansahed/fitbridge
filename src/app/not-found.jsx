  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-950 px-6 py-12 relative overflow-hidden select-none transition-colors duration-300">
      ...
      <div className="relative max-w-lg w-full text-center flex flex-col items-center z-10 p-8 sm:p-12 rounded-3xl bg-white/40 dark:bg-white/2 backdrop-blur-xl border border-white/60 dark:border-white/5 shadow-[0_20px_50px_rgba(0,0,0,0.05)] dark:shadow-[0_0_50px_rgba(0,0,0,0.3)]">
        {/* Animated outer tracking ring and central compass holder */}
        <div className="relative w-24 h-24 sm:w-28 sm:h-28 flex items-center justify-center mb-6">
          ...
          <div className="absolute inset-3 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/50 dark:border-white/10 shadow-sm dark:shadow-lg flex items-center justify-center">
            <LuCompass className="w-10 h-10 sm:w-12 sm:h-12 text-blue-600 dark:text-blue-400 animate-pulse" />
          </div>
        </div>

        {/* 404 Heading with 3D gradient text treatment */}
        <div className="relative">
          <h1 className="text-8xl sm:text-9xl font-black tracking-tighter bg-linear-to-b from-slate-800 via-blue-600 to-green-600 dark:from-white dark:via-blue-400 dark:to-green-500 bg-clip-text text-transparent drop-shadow-sm dark:drop-shadow-[0_10px_20px_rgba(0,0,0,0.5)]">
            404
          </h1>
        </div>

        <h2 className="text-xl sm:text-2xl font-bold text-slate-800 dark:text-zinc-100 mt-2 mb-3 tracking-tight">
          Lost in Digital Space?
        </h2>
        <p className="text-sm text-slate-500 dark:text-zinc-400 leading-relaxed mb-8 max-w-sm mx-auto">
          The coordinate you requested points to a void. Don&apos;t worry, you
          can sync back to safety instantly.
        </p>

        {/* Action button routing back to the core platform dashboard */}
        <Link href="/">
          <Button className="w-full sm:w-auto bg-linear-to-r from-green-600 to-blue-600 text-white font-semibold text-sm tracking-wide transition-all duration-300 px-10 py-6 shadow-[0_4px_20px_rgba(14,165,233,0.25)] hover:shadow-[0_4px_30px_rgba(34,197,94,0.45)] hover:-translate-y-0.5 rounded-md hover:rounded-3xl">
